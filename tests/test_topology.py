...
def test_build_topology_includes_gateway_and_vlan():
    devices = [
        Device(ip="192.168.0.1", mac="aa:bb:cc:dd:ee:01", hostname="gateway", device_type="router", discovery_methods=["snmp"]),
        Device(ip="192.168.0.10", mac="aa:bb:cc:dd:ee:10", hostname="switch1", device_type="switch", discovery_methods=["lldp"], snmp_info={"vlan": 100}),
        Device(ip="192.168.0.20", mac="aa:bb:cc:dd:ee:20", hostname="host1", device_type="host", discovery_methods=["arp"]),
    ]
    topology = build_topology(devices, {"subnet": "192.168.0.0/24", "gateway": "192.168.0.1"})

    assert isinstance(topology, dict)
    assert any(node["type"] == "network" for node in topology["nodes"])
    assert any(node["id"] == "aa:bb:cc:dd:ee:01|192.168.0.1" for node in topology["nodes"])
...
def test_diff_topology_detects_added_and_removed():
    ...
    current = {
        ...
        "edges": [{"source": "network", "target": "device1", "relationship": "CONNECTED_TO"}],
    }
