...
def compare_snapshots(
    previous: list[Device], current: list[Device]
) -> list[InventoryEvent]:
    ...
    for key, device in current_by_key.items():
        ...
        if old is None:
            events.append(
                InventoryEvent(
                    "NEW_DEVICE", key, f"Nuovo dispositivo rilevato: {device.ip}"
                )
            )
            ...
            if old_by_mac and old_by_mac.ip != device.ip:
                events.append(
                    InventoryEvent(
                        "IP_CHANGED",
                        key,
                        f"IP cambiato per {device.mac}: {old_by_mac.ip} -> {device.ip}",
                    )
                )
            continue
    ...
    for key, device in previous_by_key.items():
        if key not in current_by_key:
            events.append(
                InventoryEvent(
                    "DEVICE_OFFLINE", key, f"Dispositivo non piu rilevato: {device.ip}"
                )
            )
...
def _compare_device(old: Device, new: Device, key: str) -> list[InventoryEvent]:
    ...
    for port in sorted(new_ports - old_ports):
        events.append(
            InventoryEvent("PORT_OPENED", key, f"Porta aperta su {new.ip}: {port}")
        )
    for port in sorted(old_ports - new_ports):
        events.append(
            InventoryEvent("PORT_CLOSED", key, f"Porta chiusa su {new.ip}: {port}")
        )
    if old.hostname != new.hostname:
        events.append(
            InventoryEvent(
                "HOSTNAME_CHANGED",
                key,
                f"Hostname cambiato: {old.hostname} -> {new.hostname}",
            )
        )
    if old.vendor != new.vendor:
        events.append(
            InventoryEvent(
                "VENDOR_CHANGED", key, f"Vendor cambiato: {old.vendor} -> {new.vendor}"
            )
        )
    if old.device_type != new.device_type:
        events.append(
            InventoryEvent(
                ...
                key,
                f"Classificazione cambiata: {old.device_type} -> {new.device_type}",
            )
        )
    if old.security_score != new.security_score:
        events.append(
            InventoryEvent(
                ...
                key,
                f"Security score cambiato: {old.security_score} -> {new.security_score}",
            )
        )
    if old.ip != new.ip:
        events.append(
            InventoryEvent("IP_CHANGED", key, f"IP cambiato: {old.ip} -> {new.ip}")
        )
    return events
