...
def hierarchical_layout(graph: nx.Graph) -> dict[str, tuple[float, float]]:
    ...
    try:
        return nx.multipartite_layout(graph)
...
def radial_layout(graph: nx.Graph, center: str | None = None) -> dict[str, tuple[float, float]]:
    """Compute a radial layout around an optional center node."""
    try:
        if center and center in graph:
            return nx.shell_layout(graph, nlist=[[center], [n for n in graph if n != center]])
