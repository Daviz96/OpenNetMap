...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--threads", type=int, default=100, help="Numero massimo di thread"
    )
    parser.add_argument(
        "--timeout", type=float, default=1.0, help="Timeout connessioni in secondi"
    )
    parser.add_argument(
        "--snmp",
        action="store_true",
    ...
    parser.add_argument(
        "--output-dir", default="reports_output", help="Cartella output report"
    )
    parser.add_argument(
        "--from-json",
        help="Rigenera report da un inventory.json esistente senza rifare la scansione",
    )
    parser.add_argument(
        "--update-oui",
        action="store_true",
    ...
    parser.add_argument(
        "--topology",
        action="store_true",
        help="Genera topology.json, topology.graphml e topology.html",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--monitor",
        action="store_true",
        help="Esegue scansioni continue e genera eventi",
    )
    parser.add_argument(
        "--interval", type=int, default=300, help="Intervallo monitor in secondi"
    )
...
def run_once(
    config: ScanConfig, args: argparse.Namespace
...
def run_dashboard(db_path: str, host: str, port: int) -> int:
    ...
    try:
        ...
        CONSOLE.print(
            "Installa le dipendenze aggiornate con: pip install -r requirements.txt"
        )
        return 1
